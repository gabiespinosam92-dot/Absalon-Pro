...
class ClienteDetalle {
    ...
    async render() {
        ...
        const total = this.presupuestos.reduce((s,p)=>{

    if(p.total){

        return s + Number(p.total);

    }

    const subtotal=(p.items||[]).reduce((t,i)=>{

        return t + Number(i.precio||0)*Number(i.cantidad||1);
    ...
    return s + subtotal;
        ...
        if (this.presupuestos.length) {
            ultimo = [...this.presupuestos].sort((a,b)=>(Date.parse(a.fecha)||0)-(Date.parse(b.fecha)||0))  
      }
        ...
        document.querySelectorAll(".btnDuplicar").forEach(btn => {
            btn.onclick = async (e) => {
                ...
                if(typeof instanciaHistorial.duplicarPresupuesto==="function"){

                await instanciaHistorial.duplicarPresupuesto(idPresupuesto);

                }else{

                 alert("La función duplicarPresupuesto todavía no está implementada.");
